...
class AssetsApi(object):
    ...
    def list_asset_playback_ids(self, asset_id, **kwargs):  # noqa: E501
        """List an asset's playback IDs  # noqa: E501

        Gets the Playback IDs associated with the asset   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.list_asset_playback_ids(asset_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str asset_id: The asset ID. (required)
        :return: ListAssetPlaybackIDsResponse
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.list_asset_playback_ids_with_http_info(asset_id, **kwargs)  # noqa: E501
        else:
            (data) = self.list_asset_playback_ids_with_http_info(asset_id, **kwargs)  # noqa: E501
            return data

    def list_asset_playback_ids_with_http_info(self, asset_id, **kwargs):  # noqa: E501
        """List an asset's playback IDs  # noqa: E501

        Gets the Playback IDs associated with the asset   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.list_asset_playback_ids_with_http_info(asset_id, async_req=True)
        >>> result = thread.get()
        ...
        for key, val in six.iteritems(local_var_params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method list_asset_playback_ids" % key
                )
            local_var_params[key] = val
        del local_var_params['kwargs']
        # verify the required parameter 'asset_id' is set
        if ('asset_id' not in local_var_params or
                local_var_params['asset_id'] is None):
            raise ValueError("Missing the required parameter `asset_id` when calling `list_asset_playback_ids`")  # noqa: E501
        ...
        return self.api_client.call_api(
            '/video/v1/assets/{ASSET_ID}/playback-ids', 'GET',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='ListAssetPlaybackIDsResponse',  # noqa: E501
            auth_settings=auth_settings,
            async_req=local_var_params.get('async_req'),
            _return_http_data_only=local_var_params.get('_return_http_data_only'),  # noqa: E501
            _preload_content=local_var_params.get('_preload_content', True),
